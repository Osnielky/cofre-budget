...
4. To check the cap without sending 50 real messages, temporarily lower `DAILY_MESSAGE_CAP` to `2` in `ai-chat.controller.ts`, send 2 messages, confirm the 3rd is rejected with a `403` and the specific limit message, and that no SSE stream opens for that 3rd request (check the network tab). Restore `DAILY_MESSAGE_CAP` to `50` afterward.

- [ ] **Step 9: Commit**

```bash
git add apps/api/src/ai-agent/ai-conversations.controller.ts apps/api/src/ai-agent/ai-chat.controller.ts apps/api/src/ai-agent/ai-actions.controller.ts apps/api/src/ai-agent/ai-conversations.service.ts apps/api/src/ai-agent/ai-chat.service.ts apps/web/src/app/ask-cofre/page.tsx
