...
export default function VerifyPage() {
  ...
  useEffect(() => {
    ...
    if (q) { setEmail(q); setInfo(`Enter the 6-digit code we sent to ${q}.`); }
  }, []);
  ...
  return (
    <AuthShell
      ...
      footer={<span className="text-[12.5px]" style={{ color: 'rgba(242,241,234,0.55)' }}>Back to <AuthLink href="/login">Sign in</AuthLink></span>}
    >
      <form onSubmit={handleVerify} className="w-full flex flex-col gap-5">
        ...
        <button type="button" onClick={handleResend} className="text-[12px] mt-1" style={{ color: '#DDB877', cursor: 'pointer' }}>
          Resend code
        </button>
      </form>
    </AuthShell>
